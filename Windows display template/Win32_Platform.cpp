...
LRESULT CALLBACK windowCallback(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (uMsg) {
		...
		case WM_SIZE: {
			...
			renderState.sizeChangeWidth = 0.01 * (float(renderState.width) / 12.64);
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	...
	//Create Window
	HWND window = CreateWindow(windowClass.lpszClassName, L"Game Name Here (platformer)", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, 1280, 720, 0, 0, hInstance, 0);
